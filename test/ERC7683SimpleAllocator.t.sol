...
import {ERC7683SimpleAllocator} from 'src/allocators/ERC7683SimpleAllocator.sol';
...
import {IERC7683SimpleAllocator} from 'src/interfaces/IERC7683SimpleAllocator.sol';
...
abstract contract MocksSetup is Test, TestHelper {
    ...
    ERC7683SimpleAllocator erc7683SimpleAllocator;
    bytes12 usdcLockTag;
    uint256 usdcId;
    ...
    function setUp() public virtual {
        ...
        erc7683SimpleAllocator = new ERC7683SimpleAllocator(address(compactContract), 5, 100);

        usdcLockTag = _toLockTag(address(erc7683SimpleAllocator), defaultScope, defaultResetPeriod);
        usdcId = _toId(defaultScope, defaultResetPeriod, address(erc7683SimpleAllocator), address(usdc));
        (attacker, attackerPK) = makeAddrAndKey('attacker');
        defaultNonce = uint256(bytes32(abi.encodePacked(user, uint96(1))));

        ORDERDATA_GASLESS_TYPEHASH = erc7683SimpleAllocator.ORDERDATA_GASLESS_TYPEHASH();
        ORDERDATA_TYPEHASH = erc7683SimpleAllocator.ORDERDATA_TYPEHASH();
    }
}
...
abstract contract GaslessCrossChainOrderData is CompactData {
    ...
    function setUp() public virtual override {
        ...
        gaslessCrossChainOrder = IOriginSettler.GaslessCrossChainOrder({
            originSettler: address(erc7683SimpleAllocator),
            ...
            orderDataType: erc7683SimpleAllocator.ORDERDATA_GASLESS_TYPEHASH(),
            orderData: abi.encode(
                IERC7683SimpleAllocator.OrderDataGasless({
...
abstract contract OnChainCrossChainOrderData is CompactData {
    ...
    function setUp() public virtual override {
        ...
        onchainCrossChainOrder = IOriginSettler.OnchainCrossChainOrder({
            fillDeadline: uint32(_getFillExpiration()),
            orderDataType: erc7683SimpleAllocator.ORDERDATA_TYPEHASH(),
            orderData: abi.encode(
                IERC7683SimpleAllocator.OrderData({
                    arbiter: compact_.arbiter,
...
contract ERC7683SimpleAllocator_open is OnChainCrossChainOrderData {
    function test_revert_InvalidOrderDataType() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IERC7683SimpleAllocator.InvalidOrderDataType.selector,
                falseOrderDataType,
                erc7683SimpleAllocator.ORDERDATA_TYPEHASH()
            )
        );
        erc7683SimpleAllocator.open(onChainCrossChainOrder_);
    }
    ...
    function test_revert_InvalidRegistration() public {
        ...
        vm.prank(user);
        vm.expectRevert(abi.encodeWithSelector(IERC7683SimpleAllocator.InvalidRegistration.selector, user, claimHash));
        erc7683SimpleAllocator.open(onChainCrossChainOrder_);
    }
    ...
    function test_successful() public {
        ...
        vm.prank(user);
        vm.expectEmit(true, false, false, true, address(erc7683SimpleAllocator));
...
contract ERC7683SimpleAllocator_openFor is GaslessCrossChainOrderData {
    function test_revert_InvalidOrderDataType() public {
        // Order data type is invalid
        bytes32 falseOrderDataType = keccak256('false');
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IERC7683SimpleAllocator.InvalidOrderDataType.selector,
                falseOrderDataType,
                erc7683SimpleAllocator.ORDERDATA_GASLESS_TYPEHASH()
            )
        );
        ...
        erc7683SimpleAllocator.openFor(falseGaslessCrossChainOrder, signature, '');
    }
    ...
    function test_revert_InvalidOriginSettler() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IERC7683SimpleAllocator.InvalidOriginSettler.selector,
                falseOriginSettler,
                address(erc7683SimpleAllocator)
            )
        );
        (IOriginSettler.GaslessCrossChainOrder memory falseGaslessCrossChainOrder, bytes memory signature) =
        _getGaslessCrossChainOrder(
    ...
    function test_revert_InvalidNonce() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IERC7683SimpleAllocator.InvalidNonce.selector, compact_.nonce));
        (IOriginSettler.GaslessCrossChainOrder memory falseGaslessCrossChainOrder, bytes memory signature) =
        _getGaslessCrossChainOrder(
            address(erc7683SimpleAllocator),
            compact_,
    ...
    function test_revert_InvalidSponsorSignature() public {
        ...
        vm.prank(user);
        vm.expectRevert(abi.encodeWithSelector(IERC7683SimpleAllocator.InvalidSignature.selector, user, attacker));
        erc7683SimpleAllocator.openFor(gaslessCrossChainOrder_, sponsorSignature, '');
    }
    ...
    function test_revert_NonceAlreadyInUse() public {
        ...
        erc7683SimpleAllocator.openFor(gaslessCrossChainOrder2, sponsorSignature2, '');
    }
}
...
contract ERC7683SimpleAllocator_isValidSignature is OnChainCrossChainOrderData, GaslessCrossChainOrderData {
    ...
    function test_revert_InvalidLock() public {
        ...
        Claim memory claim = Claim({
            allocatorData: abi.encode(
                erc7683SimpleAllocator.QUALIFICATION_TYPEHASH(), defaultTargetBlock, defaultMaximumBlocksAfterTarget
            ),
            sponsorSignature: '',
            sponsor: user,
            nonce: defaultNonce,
...
contract ERC7683SimpleAllocator_resolveFor is GaslessCrossChainOrderData {
    function test_resolve_successful() public {
        ...
        IOriginSettler.ResolvedCrossChainOrder memory resolved =
            erc7683SimpleAllocator.resolveFor(gaslessCrossChainOrder_, '');
...
contract ERC7683SimpleAllocator_resolve is OnChainCrossChainOrderData {
    function test_resolve_successful() public {
        (IOriginSettler.OnchainCrossChainOrder memory onChainCrossChainOrder_) = _getOnChainCrossChainOrder();
        IOriginSettler.Output[] memory maxSpent = new IOriginSettler.Output[](1);
        IOriginSettler.Output[] memory minReceived = new IOriginSettler.Output[](1);
        ...
        IOriginSettler.ResolvedCrossChainOrder memory resolved = erc7683SimpleAllocator.resolve(onChainCrossChainOrder_);
        assertEq(resolved.user, resolvedCrossChainOrder.user);
        assertEq(resolved.originChainId, resolvedCrossChainOrder.originChainId);
        assertEq(resolved.openDeadline, resolvedCrossChainOrder.openDeadline);
        assertEq(resolved.fillDeadline, resolvedCrossChainOrder.fillDeadline);
...
contract ERC7683SimpleAllocator_getCompactWitnessTypeString is MocksSetup {
    function test_getCompactWitnessTypeString() public view {
        assertEq(
            erc7683SimpleAllocator.getCompactWitnessTypeString(),
            'Compact(address arbiter,address sponsor,uint256 nonce,uint256 expires,bytes12 lockTag,address token,uint256 amount,Mandate mandate)Mandate(uint256 chainId,address tribunal,address recipient,uint256 expires,address token,uint256 minimumAmount,uint256 baselinePriorityFee,uint256 scalingFactor,uint256[] decayCurve,bytes32 salt)'
        );
    }
}

contract ERC7683SimpleAllocator_checkNonce is OnChainCrossChainOrderData {
    function test_revert_invalidNonce(uint256 nonce_) public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IERC7683SimpleAllocator.InvalidNonce.selector, nonce_));
        erc7683SimpleAllocator.checkNonce(nonce_, user);
    }
    ...
    function test_checkNonce_unused(uint96 nonce_) public view {
        ...
        assertEq(erc7683SimpleAllocator.checkNonce(nonce, user), true);
    }
    ...
    function test_checkNonce_used() public {
        ...
        vm.assertEq(erc7683SimpleAllocator.checkNonce(defaultNonce, user), false);
        vm.stopPrank();
    }
    ...
    function test_checkNonce_fuzz(uint8 nonce_) public {
        ...
        vm.assertEq(erc7683SimpleAllocator.checkNonce(nonce, user), !sameNonce);
