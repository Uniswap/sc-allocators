...
import {AllocationRouter} from 'src/routers/AllocationRouter.sol';
...
contract AllocationRouterTest is Test, TestHelper {
    TheCompact compact;
    ...
    // BatchActivation typehash without witness (from the-compact/src/types/EIP712Types.sol)
    ...
    // PermitBatchWitnessTransferFrom typehash (no witness)
    ...
    function setUp() public {
        // Deploy TheCompact at hardcoded address
        compact = DeployTheCompact(new DeployTheCompact()).deployTheCompact();
        assertEq(address(compact), AllocatorLib.THE_COMPACT);
        ...
        // Deploy allocator with signer
        allocator = new HybridAllocator(signer);

        // Deploy router
        router = new AllocationRouter();
        ...
        defaultExpiration = block.timestamp + 1 days;

        // Fund sponsor
        deal(sponsor, 10 ether);
        usdc.mint(sponsor, 10 ether);
        dai.mint(sponsor, 10 ether);
    ...
    /// @dev Computes the typeHash the same way as AllocationRouter._depositRegisterAndAllocate
    function _computeRouterTypeHash(string memory witness) internal pure returns (bytes32) {
        return keccak256(
            abi.encodePacked(
                BATCH_COMPACT_TYPESTRING_FRAGMENT_ONE,
                BATCH_COMPACT_TYPESTRING_FRAGMENT_TWO,
    ...
    /// Lock struct: (bytes12 lockTag, address token, uint256 amount)
    ...
    function _createPermit2Signature(
        ...
        // The activator is the router (msg.sender to TheCompact)
        bytes32 activationHash =
            keccak256(abi.encode(BATCH_COMPACT_BATCH_ACTIVATION_TYPEHASH, address(router), idsHash, claimHash));
    ...
    /* ====================================================================== */
    /*                     Tests for depositRegisterAndAllocate               */
    /* ====================================================================== */

    function test_depositRegisterAndAllocate_simple_singleERC20() public {
        ...
        // Compute ids that the router will compute
        uint256[] memory ids = new uint256[](1);
        ids[0] = AllocatorLib.toId(lockTag, address(usdc));

        // The router computes the claimHash internally using the nonce returned from prepareAllocation
        // We need to predict what that nonce will be (allocator.nonces() + 1 = 0 + 1 = 1)
        uint256 expectedNonce = 1;

        // Compute commitment hashes using proper Lock struct encoding
        bytes32[] memory commitmentHashes = new bytes32[](1);
        commitmentHashes[0] = _computeCommitmentHash(ids[0], defaultAmount);

        // For empty witness, router uses BATCH_COMPACT_TYPEHASH (no Mandate)
        bytes32 typeHash = BATCH_COMPACT_TYPEHASH;

        // Compute claimHash WITHOUT witnessHash (empty witness case)
        ...
        bytes memory signature = _createPermit2Signature(permitted, details, claimHash, sponsorPrivateKey);

        // Execute
        router.depositRegisterAndAllocate(sponsor, arbiter, permitted, details, bytes32(0), '', signature);
        vm.snapshotGasLastCall('depositRegisterAndAllocate_simple_singleERC20');

        // Verify allocator nonce incremented
        ...
        assertTrue(
            allocator.isClaimAuthorized(claimHash, arbiter, sponsor, expectedNonce, details.deadline, idsAndAmounts, '')
        );

        // Verify tokens are in compact
        assertEq(usdc.balanceOf(address(compact)), defaultAmount);
        assertEq(compact.balanceOf(sponsor, ids[0]), defaultAmount);
    }

    function test_depositRegisterAndAllocate_explicit_singleERC20() public {
        bytes12 lockTag = _getLockTag();
        uint256 nonce = 1;
        ...
        // For empty witness, use BATCH_COMPACT_TYPEHASH (no Mandate)
        bytes32 typeHash = BATCH_COMPACT_TYPEHASH;

        // The explicit function uses the allocator's nonce after prepareAllocation increments it
        // prepareAllocation returns nonces + 1 = 0 + 1 = 1
        uint256 expectedNonce = 1;

        // Compute claimHash WITHOUT witnessHash (since witness is empty, bytes32(0))
        // AllocatorLib.getClaimHash omits the witness when it's bytes32(0)
        bytes32 claimHash = keccak256(
            abi.encode(
                typeHash,
                arbiter,
        ...
        // Execute
        router.depositRegisterAndAllocate(
            sponsor, arbiter, permitted, details, bytes32(0), '', signature, claimHash, address(allocator), typeHash
        );
        vm.snapshotGasLastCall('depositRegisterAndAllocate_explicit_singleERC20');
    ...
    function test_depositRegisterAndAllocate_simple_multipleERC20() public {
        bytes12 lockTag = _getLockTag();
        uint256 nonce = 1;
        uint256 amount1 = defaultAmount;
        uint256 amount2 = defaultAmount / 2;

        // Prepare token permissions (sorted by address)
        ISignatureTransfer.TokenPermissions[] memory permitted;
        ...
        // Expected nonce from prepareAllocation
        uint256 expectedNonce = 1;
        ...
        // Execute
        router.depositRegisterAndAllocate(sponsor, arbiter, permitted, details, bytes32(0), '', signature);
        vm.snapshotGasLastCall('depositRegisterAndAllocate_simple_multipleERC20');
    ...
    function test_depositRegisterAndAllocate_simple_nativeToken() public {
        bytes12 lockTag = _getLockTag();
        uint256 nonce = 1;

        // Prepare token permissions with native token (address(0))
        // Native token must be first in the array for batchDepositAndRegisterViaPermit2
        ISignatureTransfer.TokenPermissions[] memory permitted = _createTokenPermissions(address(0), defaultAmount);
        ...
        // Compute ids
        uint256[] memory ids = new uint256[](1);
        ids[0] = AllocatorLib.toId(lockTag, address(0));
        ...
        // Create Permit2 signature - for native tokens, the idsHash in the activation
        // still includes the native token id, so we pass the full permitted array
        // (TheCompact strips the native token from the Permit2 call internally)
        bytes memory signature = _createPermit2Signature(permitted, details, claimHash, sponsorPrivateKey);

        // Execute with value - need to use the explicit function which is payable
        router.depositRegisterAndAllocate{value: defaultAmount}(
            sponsor, arbiter, permitted, details, bytes32(0), '', signature, claimHash, address(allocator), typeHash
        );
        vm.snapshotGasLastCall('depositRegisterAndAllocate_simple_nativeToken');

        // Verify allocator nonce incremented
        assertEq(allocator.nonces(), 1);

        // Verify native tokens are in compact
    ...
    function test_depositRegisterAndAllocate_revert_InvalidAllocator() public {
        bytes12 invalidLockTag = bytes12(0); // Invalid lockTag with allocatorId = 0
        uint256 nonce = 1;

        // Prepare token permissions
        ISignatureTransfer.TokenPermissions[] memory permitted = _createTokenPermissions(address(usdc), defaultAmount);

        // Prepare deposit details with invalid lockTag
        DepositDetails memory details = _createDepositDetails(nonce, defaultExpiration, invalidLockTag);

        // Attempt to call should revert with InvalidAllocator
        vm.expectRevert(AllocatorLib.InvalidAllocator.selector);
        router.depositRegisterAndAllocate(sponsor, arbiter, permitted, details, bytes32(0), '', '');
    }
    ...
    function test_depositRegisterAndAllocate_fullClaimFlow() public {
        bytes12 lockTag = _getLockTag();
        uint256 nonce = 1;
        ...
        claims[0] = BatchClaimComponent({id: ids[0], allocatedAmount: defaultAmount, portions: portions});
